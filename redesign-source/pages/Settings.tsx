    ...
    <div className="px-5 lg:px-10 py-6 lg:py-8 max-w-2xl">
      ...
      <div className="p-6 rounded-2xl bg-white border border-[#E8E4DD]/60 mb-6">
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <h2 className="text-lg font-medium text-[#2A2623]">Aaron</h2>
            <p className="text-sm text-[#A8A198]">aaron@carbonproject.com.au</p>
          </div>
      ...
      <div className="space-y-6">
        ...
        <SectionCard title="Data & Privacy">
          ...
          <Divider />
          <ActionRow label="Clear Local Data" description="Remove all data stored on this device" action="Clear" destructive />
        </SectionCard>
        ...
        <SectionCard title="About">
          ...
          <div className="flex items-center justify-between py-1">
            ...
            <p className="text-sm font-['DM_Mono'] text-[#A8A198]">2.4 MB</p>
          </div>
        </SectionCard>
      </div>

      <div className="mt-8 mb-4 text-center">
        <p className="text-xs text-[#C4C0B8]">Life Design — Crafted with care for meaningful living</p>
      </div>
    </div>
  )
}
...
function ActionRow({ label, description, action, destructive }: {
  label: string; description: string; action: string; destructive?: boolean
}) {
  return (
    <div className="flex items-center justify-between py-1">
      <div>
      ...
      <button className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${destructive ? 'bg-red-50 text-red-500 hover:bg-red-100' : 'bg-[#F5F3EF] text-[#5C554C] hover:bg-[#E8E4DD]'}`}>
        {action}
      </button>
    </div>
  )
